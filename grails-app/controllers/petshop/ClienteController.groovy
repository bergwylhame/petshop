...
class ClienteController {
    ...
    def findClienteNome(){
        ...
        def clienteInstanceList = c.list {
            or {
                ilike("nome", "%${params.busca}%")
                ilike("telefone", "%${params.busca}%")
                ilike("cpf", "%${params.busca}%")
            }
            order("nome", "asc")
        }

        if(clienteInstanceList){
            render(view: "index", model: [clienteInstanceList:clienteInstanceList])
        }else{
            render(view: "index", model:[clienteInstanceList:Cliente.findAll()])
        }
    }
}
