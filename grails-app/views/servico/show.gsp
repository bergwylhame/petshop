	...
	<body>
		...
		<div id="show-servico" class="content scaffold-show" role="main">
			...
			<ol class="property-list servico">
				...
				<g:if test="${servicoInstance?.atendimento}">
				<li class="fieldcontain">
					<span id="atendimento-label" class="property-label"><g:message code="servico.atendimento.label" default="Atendimento" /></span>
					
						<g:each in="${servicoInstance.atendimento}" var="a">
						<span class="property-value" aria-labelledby="atendimento-label"><g:link controller="atendimento" action="show" id="${a.id}">${a?.encodeAsHTML()}</g:link></span>
						</g:each>
